...
string calculator::convert_to_reverse_polish_notation(const string& inf_string)
{
    vector<string> inf,post;
	stack<string> stk;
	split(inf_string, ' ', inf); //split on whitespaces
	inf = reverse_v(inf);
    int status=2; //The variable will have 3 states: 0 - conversion isn't performed, the error; 1 - conversion process is successful; 2 - conversion in process
    int i=0; 
...
{
	string temp = input.substr(VARIABLE_KEY_WORD.length(), input.length() - VARIABLE_KEY_WORD.length());
...
bool calculator::process_var_line(string& line, string& error)
{
	...
	{
		error = "Invalid variable declaration. Type 'help' to get information about variable name.";
		return false;
	}
...
void calculator::show_variables()
{
	...
	std::cout << sign3 << setw(t.length()) << t << sign3 << setw(t.length()) << t << sign3 << endl;
	std::cout << sign2 << setw(t.length()) << "Name" << sign2 << setw(t.length()) << "Value" << sign2 << endl;
	std::cout << sign3 << setw(t.length()) << t << sign3 << setw(t.length()) << t << sign3 << endl;
	
	for(auto var : variables)
	{
		p++;
		std::cout << sign2 << setw(t.length()) << var.first << sign2 << setw(t.length()) << var.second << sign2 << endl;
		if (p == variables.size())
			std::cout << sign3 << setw(t.length()) << t << sign3 << setw(t.length()) << t << sign3 << endl;
		else
			std::cout << sign2 << setw(t.length()) << t << sign2 << setw(t.length()) << t << sign2 << endl;
	}
}
...
string calculator::divide_spaces(const string& input, string& error)
{
	...
	for(int i = 0; i < temp.length(); i++)
	{
		if(is_sign(temp[i]) || temp[i] == '(' || temp[i] == ')' || temp[i] == ')')
		{
			output.push_back(' ');
			output.push_back(temp[i]);
			output.push_back(' ');
		...
		if(is_digit(temp[i]) || temp[i] == '.' || is_letter(temp[i]) || temp[i] == '_')
		{
			output.push_back(temp[i]);
			continue;
		}
...
void calculator::start()
{
	...
	while(input != "exit")
	{
		if(input == "help") 
			...
			if(regex_match(input, regex_var))
			{
				if(!process_var_line(input, error))
					std::cout << error << endl;
			}
		else
		{
